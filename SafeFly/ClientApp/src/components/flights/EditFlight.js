...
export class EditFlight extends Component {

    static displayName = EditFlight.name;
    ...
    async getFlight(flightId) {
        ...
        let dataAirline = this.state.airlines.find(a => a.airlineId == data.airline);
        let dataTakeOffAirport = this.state.airports.find(a => a.airportId == data.takeOffAirport);
        let dataLandingAirport = this.state.airports.find(a => a.airportId == data.landingAirport);

        this.setState({
            flightId: flightId,
            flightNumber: data.flightNumber,
            airline: dataAirline,
            airlineDescription: dataAirline.airlineName,
            price: data.price,
            status: data.status,
            statusDescription: data.status,
            numberOfPassengers: data.numberOfPassengers,
            takeOffAirport: dataTakeOffAirport,
            takeOffAirportDescription: dataTakeOffAirport.airportName,
            takeOffDate: new Date(data.takeOffDate),
            landingAirport: dataLandingAirport,
            landingAirportDescription: dataLandingAirport.airportName,
            landingDate: new Date(data.landingDate),
            loading: false
        });
    }
    ...
    async handleSubmit(event) {
        ...
        await fetch(`flights/edit-flight`, requestOptions);
    ...
    render() {
        return (
            <Container className="editFlight">
                <h2>Edit flight</h2>
                <Form>
                    <Col>
                        <FormGroup row>
